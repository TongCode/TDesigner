    ...
    function (require) {
        ...
        var domHandlers = {
            ...
            click: function (event, manually) {
                ...
                console.log("点击事件",event,manually);
            ...
            mousemove: function (event, manually) {
                //console.log("mousemove移入事件：",event,manually);
                if (! isZRenderElement(event) && ! manually) {
                    return;
                }
        ...
        Handler.prototype._dispatchAgency = function (targetShape, eventName, event, draggedShape) {
            ...
            console.log("_dispatchAgency："+eventHandler);
            var eventPacket = {
                type : eventName,
                event : event,
                target : targetShape,
        ...
        Handler.prototype._iterateAndFindHover = (function() {
            ...
            return function() {
                ...
                console.log("当前图形集合："+list.length,list);
                var currentZLevel;
                var currentLayer;
                var tmp = [ 0, 0 ];
                for (var i = list.length - 1; i >= 0 ; i--) {
                    var shape = list[i];
                    console.log("当前图形所在层：",shape.zlevel,currentZLevel);
